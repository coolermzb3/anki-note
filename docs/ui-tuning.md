...
- 谱页先按小节线和谱表切换把短时值音符分成连续 run，再由 VexFlow 在每个 run 内尽量连梁；至少两个同谱表音符即可连接，单个音保留独立符尾。连梁不跨小节线，线条保持中性色，音符自身仍按答题状态着色。全音符和四分音符每 4 音画一条小节线，八分音符和十六分音符每 8 音画一条小节线；小节线按前后可见音符的实际边界放在空隙中点，没有安全空隙时省略，避免穿过符尾。
