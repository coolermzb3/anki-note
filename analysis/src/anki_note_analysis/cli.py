...
from .backup import latest_target_note_ids, load_backup, prepare_output_dir, qualified_reviews
from .metrics import (
    bootstrap_recent_metrics,
...
def run(
    ...
    window_comparison = compare_metric_windows(
    ...
    stability, threshold_summary = bootstrap_recent_metrics(recent, note_ids, iterations=bootstrap_iterations)
    metric_alignment = cross_day_metric_alignment(target_scheduler_history, note_ids, active_days)
    allocation, policy_summary = candidate_policy_distributions(target_scheduler_history, recent, note_ids)
    ...
    hyperparameters = hyperparameter_sensitivity(
        target_scheduler_history,
        recent,
        note_ids,
        draw_count=replay_draw_count,
    ...
    maintenance_gaps = maintenance_gap_sensitivity(
        target_scheduler_history,
        recent,
        note_ids,
        draw_count=replay_draw_count,
    ...
    cold_start = cold_start_sensitivity(
        target_scheduler_history,
        recent,
        note_ids,
        repetitions=max(50, replay_repetitions),
